...
export default function PersonalCreate({ active, setActive }: IProps) {
      ...
      <div className={active ? `${styles.popup_active}` : `${styles.popup}`} onClick={() => setActive(false)}>
        ...
        >
          <form onSubmit={handleSubmit} className={styles.formCreate}>
            <div className={styles.popupName}>Додати працівника</div>
            ...
            <input type="text" placeholder="Пароль" value={password} onChange={(e) => setPassword(e.target.value)} className={styles.input}/>
            <input type="text" placeholder="Посада" value={role} onChange={(e) => setRole(e.target.value)} className={styles.input}/>
