...
export default function OrderDelete({ active, setActive, orderId }: IProps) {
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();

    try {
      const response = await axios.post('http://localhost:3001/order/delete', {
        id: orderId,
      });
